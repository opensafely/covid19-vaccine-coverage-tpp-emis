...
def run(spec_path):
    ...
    out_path = (
        Path(os.path.abspath(__file__)).parents[1] / "analysis" / "add_groupings_2.py"
    )

    with open(out_path, "w") as f:
        write_add_groupings(list(get_groupings(spec)), f)
...
"""
    ...
    for g in groupings:
        f.write(f'    row["{g["column"]}"] = add_{g["column"]}(row)\n')

    for g in groupings:
        f.write(f"def add_{g['column']}(row):\n")
...
def render_expr(expr):
    ...
    if op == "null":
        val = operands[0]
        return f"not {render_expr(val)}"

    if op == "notnull":
        val = operands[0]
        return f"{render_expr(val)}"
